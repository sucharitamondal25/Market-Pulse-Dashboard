import { tickerData as mockTicker } from "@/data/marketData";
import type { TickerItem } from "@/hooks/useDashboard";

interface Props { ticker?: TickerItem[] | null; }

export function TickerBar({ ticker }: Props) {
  const source = ticker && ticker.length > 0 ? ticker : mockTicker;
      ...
      <div className="ticker-animate">
        {doubled.map((item, i) => (
          <span key={i} className="inline-flex items-center gap-1 px-4 text-xs whitespace-nowrap">
            ...
            <span className={item.up ? "neon-green" : "neon-red"}>{item.change}</span>
